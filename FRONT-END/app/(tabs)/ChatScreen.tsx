...
function buildSystemPrompt(child: ChildProfile): string {
  const foods = child.foods
    .map((f) => `• ${f.name}: etapa "${f.sosStep}"`)
    .join("\n");

  return `Você é o Assistente Juca, especializado em introdução alimentar terapêutica para crianças.

Você domina:
- Método SOS (Sequential Oral Sensory): hierarquia sensorial cheirar → tocar → beijar → provar → comer.
- ABA (Applied Behavior Analysis): reforço positivo, prompts graduais e extinção de recusa.
- Food Chaining: parte de alimentos aceitos, variando textura, temperatura e sabor progressivamente.

Perfil da criança:
Nome: ${child.name}, ${child.age} meses.
Progresso SOS atual:
${foods}

Responda de forma acolhedora, clara e prática. Use o nome "${child.name}" quando pertinente.
Respostas entre 3 e 6 parágrafos curtos. Nunca diagnostique condições médicas.
Se necessário, indique avaliação profissional gentilmente.`;
}
...
export default function ChatScreen({ childProfile = MOCK_CHILD }: Props) {
  ...
  return (
    <SafeAreaView style={styles.safe}>
      ...
      {/* ── Header: "Assistente Juca" em preto, sem logo ── */}
